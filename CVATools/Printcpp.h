...
namespace Utilities
{
    ...
    {
    public:
        PrintCpp(const std::string & cFileName, bool bAppend, std::size_t iPrecision) : cFileName_(cFileName), bAppend_(bAppend), iPrecision_(iPrecision)
        {}
        
        template<class C> void PrintInFile(const std::vector<C> & dData)
        {
            std::ofstream myfile;
            myfile.precision (iPrecision_);
