...
namespace Finance
{
    namespace Market
    {
        ...
        FXMarket::FXMarket(const std::string & cMarketName, const Utilities::Date::MyDate & today) : Market(today), DomesticRateMarket(today), ForeignRateMarkets(std::vector<RateMarket>(0, RateMarket(today)))
        {
            Load(cMarketName);
        }
